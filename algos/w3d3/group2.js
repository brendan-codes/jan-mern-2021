...
function webMD(ailments, meds) {
    const Output =[];
    let match=false;
    //Filter each medication
    const medication = meds.filter(medication => {
        //Filter by each ailment
            ailments.map(ailment => {
                match = medication.treatableSymptoms.includes(ailment)

            })
            return match
            medication.map(medication => {Output.push(medication.name)
