...
function findObjectsFilter(search, items) {
    ...
    //these capture the property values to be compared against the propety values in items[]
    const firstKey_val = search.firstName;
    console.log("firstKey_val", firstKey_val); // Bob

    const key2_val = search.age;
    console.log("key2_val", key2_val); // 31

    // items.hasOwnProperty("firstName")
    if(items.hasOwnProperty(firstName)==firstKey_val)
