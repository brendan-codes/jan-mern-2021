...
function getMaxServings(recipe, ingredients) {
  let count = Infinity;
  let recipeKeys = Object.keys(recipe); // returns an array of the keys
  for (let i = 0; i < recipeKeys.length; i++) {
      if (!available.hasOwnProperty(recipeKeys[i])) {
          return 0;
      }
      // math.min returns the lower of the 2 nums passed into it
      count =  Math.min(Math.floor(available[recipeKeys[i]] / recipe[recipeKeys[i]]), count);
  }
  return count;
}
